...
class Attention(nn.Module):
    def __init__(self, dim, num_heads, bias):
        ...
        self.qkv_c = nn.Conv2d(dim, dim*3, kernel_size=1, bias=bias)
        self.qkv_dwconv_c = nn.Conv2d(dim*3, dim*3, kernel_size=3, stride=1, padding=1, groups=dim*3, bias=bias)
        self.project_out_c = nn.Conv2d(dim, dim, kernel_size=1, bias=bias)

        # Channel Reducer
        self.reduce_channels = nn.Conv2d(dim*2, dim, kernel_size=1, bias=bias)
                
    def forward(self, x):
        b, c, h, w = x.shape

        # Channel wise attention
        q_c, k_c, v_c = self.qkv_dwconv_c(self.qkv_c(x)).chunk(3, dim=1)
