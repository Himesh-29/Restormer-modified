...
class Attention(nn.Module):
    def __init__(self, dim, num_heads, bias):
        ...
        self.q1X1_1_cV = nn.Conv2d(dim, dim, kernel_size=1, bias=bias)
        self.q1X1_1_cD = nn.Conv2d(dim, dim, kernel_size=1, bias=bias)

        self.q1X1_2_cA = nn.Conv2d(dim, dim, kernel_size=1, bias=bias)
    ...
    def forward(self, x):
        ...
        qw = rearrange(qw, 'b (head c) h w -> b head c (h w)', head=self.num_heads) 
        ...
        outw = rearrange(outw, 'b head c (h w) -> b (head c) h w', head=self.num_heads, h=h, w=w)

        outw = self.project_outw(outw)
